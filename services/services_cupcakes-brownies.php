...
$wantedCategories = ['Cupcakes', 'Brownies'];
...
$query = "
  ...
  FROM services
  WHERE category IN (" . implode(',', array_fill(0, count($wantedCategories), '?')) . ")
    AND status = 'enabled'
  ORDER BY service_name
";
$stmt = $pdo->prepare($query);
$stmt->execute($wantedCategories);
    ...
    <div class="container py-5" style="padding-top: 100px !important;">
        ...
        <div class="card-container">
            <?php if (empty($services)): ?>
                <p class="text-muted">No Cupcakes or Brownies available right now.</p>
            <?php else: ?>
